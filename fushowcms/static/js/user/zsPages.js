			...
			return (function(){
				obj.on("click","a.tcdNumber",function(){
					...
					if(typeof(args.backFn)=="function"){
						args.backFn(current);				
					var url = "/user/findByUserIdGiveSlz"; 
					reqAjax(url,{UID:uid,page:current,rows:10},function(msg) {
						localStorage.setItem("zspage",current);
						$(".removediv").remove();
						console.log(msg);
				...
				obj.on("click","a.prevPage",function(){
					...
					reqAjax(url,{UID:args.id,page:current-1,rows:10},function(msg) { 
						localStorage.setItem("zspage",current-1);
						$(".removediv").remove();
						$.each(msg.Data.data, function(key,val){ 	
							if(uid == val.RecipientId){
								val.Num = "+" + val.Num;

			...
			return (function(){
				obj.on("click","a.tcdNumber",function(){
					...
					if(typeof(args.backFn)=="function"){
						args.backFn(current);				
					var url = "/user/findByRoomIdAll"; 
					reqAjax("/user/findByRoomIdAll",{RoomId:args.roomId,page:current,rows:20},function(msg) {
						localStorage.setItem("glypage",current);
						$(".removediv").remove();
						$.each(msg.Data.data, function(key, val) {
							var str = "<tr class='jjj removediv'><td>" + val.ModifyBy + "</td><td>" + val.ModifyTime + "</td><td><button class='cancelmanage'  onclick=''  data-rowid='" + val.Id + "'>撤销</button></td></tr>";
				...
				obj.on("click","a.prevPage",function(){
					...
					//var url = "/user/findByRoomIdAll";  
					reqAjax("/user/findByRoomIdAll",{RoomId:args.roomId,page:current-1,rows:20},function(msg) {
						localStorage.setItem("glypage",current+1);
						$(".removediv").remove();
						$('.jjj').remove();
						$.each(msg.Data.data, function(key, val) {
							var str = "<tr class='jjj removediv'><td>" + val.ModifyBy + "</td><td>" + val.ModifyTime + "</td><td><button class='cancelmanage'  onclick=''  data-rowid='" + val.Id + "'>撤销</button></td></tr>";
				...
				obj.on("click","a.nextPage",function(){
					...
					//var url = "/user/findByRoomIdAll";  
					// 
					reqAjax("/user/findByRoomIdAll",{RoomId: roomId,page:current+1,rows:20},function(msg) {
						localStorage.setItem("glypage",current+1);
						$(".removediv").remove();
						$('.jjj').remove();
						$.each(msg.Data.data, function(key, val) {

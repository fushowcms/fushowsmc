			...
			return (function(){
				obj.on("click","a.tcdNumber",function(){
					...
					if(typeof(args.backFn)=="function"){
						args.backFn(current);
						reqAjax("/user/settlementdetail",{UID:args.id,page:current,rows:20},function(msg){
							if(msg.ErrorCode!=0) {
								// Dialog(msg.ErrorMsg,true,"确定",null,function() {
								// 	$('.dialog').remove();
								// },null);
							}else {
								setStorage("sydhpage",current);
	//							window.location.reload();
				...
				obj.on("click","a.prevPage",function(){
					...
					if(typeof(args.backFn)=="function"){
						...
						reqAjax("/user/settlementdetail",{UID:args.id,page:current-1,rows:20},function(msg){
							 if(msg.ErrorCode!=0) {
							 	Dialog(msg.ErrorMsg,true,"确定",null,function() {
							 		$('.dialog').remove();
							 	},null);
							}else {
								setStorage("sydhpage",current-1);
	//							window.location.reload();
								$(".removediv").remove();
								$.each(msg.Data.rows, function(key,val){ 	
								if(val.IsCashing) {

...
//		router.GET("/user/:id", func(c *fushow.FushowContext) {
...
func (c *Context) GetQuery(key string) (string, bool) {
	...
	if values, ok := req.URL.Query()[key]; ok && len(values) > 0 {
		if strings.ToLower(key) == "uid" {
			if values[0] != "" {
				//				fmt.Println("************* GetQuery s->" + values[0])
				var decVal = getDecValue(values[0], "fushow.cms")
				//				fmt.Println("************* GetQuery d->" + decVal)
				fmt.Println("[GIN_GET] key:", key, "src:", values[0], "dsecvalue:", decVal)
				return decVal, true
			}
		}
		fmt.Println("[GIN_GET] key:", key, "value:", values[0])
...
// SubEndstr 截取字符串 start 起点下标 end 终点下标(不包括)
...
func (c *Context) PostForm(key string) string {
	...
	if strings.ToLower(key) == "uid" {
		if value != "" {
			//			fmt.Println("************* PostForm s->" + value)
			var decVal = getDecValue(value, "fushow.cms")
			//			fmt.Println("************* PostForm d->" + decVal)
			fmt.Println("[GIN_POST] key:", key, "src:", value, "dsecvalue:", decVal)
			return decVal
		}
	}
	fmt.Println("[GIN_POST] key:", key, "value:", value)
	return value
}
